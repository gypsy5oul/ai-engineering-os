...
| `execution` | `inline`, `subagent` or `team`. Whether it ran detached is `execution.runtime.background`, a property of the runtime rather than a fourth mode. Where the work runs is a separate field, `isolation`; see [execution](execution.md) |
