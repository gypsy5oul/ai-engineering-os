...
| `v0.45.4.json` | 0.45.3 | 2.1.263 | no | **13 of 14 probes pass, none fail.** The worktree lifecycle completes inside a certification for the first time — created, committed, merged, removed. Every mechanism the harness can reach is now exercised on real evidence. The one not-run is teams, which have no CLI surface. |

## A correction that applies to more than one run

Every `worktree-isolation-was-actually-used` pass recorded before 2026-09-04 is false. The probe counted `worktree_created` events; the events fired whether or not a worktree existed, because `WorktreeCreate` is a provider hook and this plugin's recorder was replacing the platform's creation with a no-op. The affected records carry the correction in their own `notes` and their verdicts stand as produced — certification was refused in all of them anyway, so nothing that was claimed changes. What changes is what the evidence meant.
